...
Personal memory governance MCP server. Part of `memory-system`（旧名 `project-management-personal`，2026-05-16 改）。
    ...
    "SessionEnd": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/path/to/memory-system/plugins/claude-code/session-end.sh"
          }
        ]
      }
    ]
...
d["mcpServers"]["memoryd"] = {
    "command": "/path/to/memory-system/memoryd/.venv/bin/memoryd-server",
    "args": [],
    "env": {
...
> 旧的 `plugins/codex/notify-wrapper.sh` 已删除。
...
# 详见下面 Phase 1 手册）
/Users/abble/memory-system/memoryd/.venv/bin/memoryd setup swap-codex-notify --to wrapper
```
